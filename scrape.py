...
def start_requests():
  ...
  #for i in range(0, 50):
    ...
    t = threading.Thread(target=get_llc_info, args=(url, owner_name, i))
    threads.append(t)
    t.start()
    t.join()

def get_llc_info(url, owner_name, i):
  soup = soupify(url)
  search_result = soup.find('td', class_='large-width')
  if fuzz.partial_ratio(owner_name, search_result.get_text()) > 80:
